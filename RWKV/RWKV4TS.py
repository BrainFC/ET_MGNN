...
from RWKV.embed import DataEmbedding
...
class RWKVBlock(nn.Module):
    ...
    def __init__(self, input_dim, layer_id):
        super().__init__()

        self.config = RWKVConfig(n_layer=2, n_head=2,n_embd=input_dim)
        config = self.config
...
class RWKV4TS(nn.Module):
    ...
    def __init__(self, config, data):
        ...
        self.rwkv = nn.ModuleList([RWKVBlock(rwkv_config, i) for i in range(rwkv_config.n_layer)])
